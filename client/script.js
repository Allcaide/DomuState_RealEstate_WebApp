...
function renderPropertyCard(property) {
  if (property.isFavorite) {
    console.log('[renderPropertyCard] Property favorito:', property.id);
  }
  return `
    <div class="card">
      <div class="relative">
        <img src="${property.images[0] || '/api/placeholder/400/300'}" alt="${property.title}" class="property-image w-full" onerror="this.onerror=null; this.src='/api/placeholder/400/300';">
        <button class="favorite-btn absolute top-3 right-3 bg-white p-2 rounded-full" data-id="${property.id}">
          <i class="fas fa-heart${property.isFavorite ? ' text-red-500' : ''}"></i>
        </button>
      </div>
      <div class="p-3">
